...
// Replace your getAllBetTypesForMatch with this
...
const getTypesByCategory = (
  match: DetailedMatch
): Record<string, { category: string; type: string }[]> => {
...
export function formatMatchLabel(
  isoUtc: string,
  options?: { locale?: string; tz?: string }
): string {
...
export function addBetToBuilder(
  ...
  const existingSelections: BetTypeSelection[] = JSON.parse(
    localStorage.getItem('betTypeSelections') || '[]'
  )
...
export default function Component({}: BookiesTableProps) {
  ...
  const fetchAllMatches = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        ...
        matches =
          (data as { matches?: BasicMatch[]; data?: BasicMatch[]; results?: BasicMatch[] })
            .matches ||
  ...
  const leagues: string[] = Array.from(
    new Set(allMatches.map((match: BasicMatch) => match.league))
  ).sort()
  ...
  const handleAnalyzeBet = (selections: BetTypeSelection[], stake: number) => {
    ...
    console.log(selections, `Selections for analysis: `)
                      ...
                      <div className="flex gap-1 overflow-x-auto">
                        {availableDateSpans.map((dateSpan) => (
                          <Button
                            ...
                            size="sm"
                            onClick={() => handleDateSpanChange(dateSpan.key)}
                            className={`
                            h-7 px-2 text-xs whitespace-nowrap transition-all duration-200
                            ${
                  ...
                  <div className="flex items-center justify-between gap-2 fold:gap-1">
                    ...
                    <div className="flex gap-1">
                      <Button
                        variant={viewMode === 'time' ? 'default' : 'outline'}
                        size="sm"
                        onClick={() => handleViewModeChangeUrl('time')}
                        className={`
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      >
                        ...
                        {hasActiveFilters && (
                          <Badge variant="secondary" className="h-3 w-3 p-0 text-[0.5rem]" />
                        )}
                      </Button>
                  ...
                  <Table className="table-fixed w-full">
                    <TableHeader className="sticky top-0 z-20 bg-white border-b shadow-sm dark:bg-kvotizza-dark-bg-20">
                      <TableRow className="bg-white dark:bg-kvotizza-dark-bg-20">
                        {isMobileView ? (
                          <TableHeadMini className="w-[80px] bg-white dark:bg-kvotizza-dark-bg-20">
                            {' '}
                        ...
                        {quickMarkets.map((market) => (
                          <TableHeadMini
                            key={market.key}
                            className="font-semibold text-left sm:pl-8 bg-white dark:bg-kvotizza-dark-bg-20 text-xs sm:text-sm dark:text-white"
                          >
                            {market.displayName}
                          </TableHeadMini>
                        ))}
                          ...
                          <React.Fragment key={match.id}>
                            ...
                            {expandedRows.includes(match.id) && (
                              <TableRow key={`${match.id}-expanded`}>
                                <TableCell
                                  colSpan={colSpan}
                                  className="bg-muted/30 p-2 fold:p-1 dark:bg-kvotizza-dark-bg-20"
                                >
                                    ...
                                    {loadingDetails[match.id] ? (
                                      ...
                                      <>
                                        ...
                                        <div className="space-y-3 fold:space-y-2 max-h-[500px] fold:max-h-[400px] overflow-y-auto">
                                          {Object.entries(
                                            getTypesByCategory(detailedMatches[match.id])
                                          ).map(([category, types]) => (
                                            <div
                                              key={category}
                                              className="space-y-2 fold:space-y-1 "
                                            >
                                              ...
                                              {types.map((betType) => (
                                                ...
                                                >
                                                  ...
                                                  <div className="grid grid-cols-4 sm:grid-cols-5 md:grid-cols-5 gap-2 fold:gap-1">
                                                    {detailedMatches[match.id].bookies.map(
                                                      (bookie: Bookie) => {
              ...
              <CardContent className="p-2 fold:p-1  dark:bg-kvotizza-dark-bg-20">
                <div className="flex flex-col gap-2 fold:gap-1">
                  ...
                  <div className="flex items-center justify-center gap-2 fold:gap-1">
                    <span className="text-xs fold:text-[0.6rem] text-muted-foreground  dark:bg-kvotizza-dark-bg-20">
                      Prikaži:
                    </span>
                    <Select
