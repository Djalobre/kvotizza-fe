...
export async function GET(request: Request) {
  try {
    ...
    const apiUrl = `${API_CONFIG.baseUrl}${apiConfig.endpoints.tipovanje_filters}${
    ...
    let filter_leagues: any[] = []

    // Handle different API response formats
    if (Array.isArray(data)) {
      filter_leagues = data
    } else if (data && typeof data === 'object') {
      filter_leagues = data.leagues || data.data || data.results || []
    }

    const filterLeaguesData: string[] = filter_leagues

    // Return simple array for client-side filtering and pagination
    return NextResponse.json(filterLeaguesData)
  } catch (error) {
