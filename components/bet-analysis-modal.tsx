...
export function BetAnalysisModal({ isOpen, onClose, selections, stake }: BetAnalysisModalProps) {
  ...
  const analyzeAcrossBookies = async () => {
    ...
    try {
      ...
      const suggestions = bonusCalculatorService.getBonusOptimizationSuggestions(
        betSelections,
        stake
      )
  ...
  const copyBestBet = () => {
    if (bookieSummaries.length > 0) {
      ...
      const bonusText = bonus
        ? ` (uključujući ${bonus.bonusPercentage}% bonus: ${bonus.bonusAmount.toFixed(2)} rsd)`
        : ''
      const betText = `Najbolja kvota: ${
        bestBet.bookie
      } - Ukupna kvota: ${bestBet.totalOdds.toFixed(
        2
      )} - Potencijaln dobitak: ${bestBet.potentialWin.toFixed(2)} rsd${bonusText}`
  ...
  return (
    <TooltipProvider>
      <Dialog open={isOpen} onOpenChange={onClose}>
        <DialogContent
          className="w-[95vw] max-w-6xl h-[95vh] max-h-[95vh] overflow-hidden flex flex-col p-0 dark:bg-kvotizza-dark-bg-10"
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 md:space-y-6">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-lg md:text-2xl font-bold text-kvotizza-green-600 dark:text-kvotizza-green-400">
                      {bookieSummaries.length > 0
                        ? `${formatNumberEuropean(bookieSummaries[0].potentialWin)} rsd`
                        : '0.00 rsd'}
                    </p>
                  </div>
                </div>
              ...
              <CardContent>
                <div className="grid gap-3 ">
                  {selections.map((selection, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-3 bg-muted/50 rounded border border-md rounded-lg dark:bg-white/5"
                    >
                      <div>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs text-kvotizza-green-700 dark:text-kvotizza-green-300 font-bold">
                          @ {detailedAnalysis[index]?.bestBookie || 'N/A'}
                        </p>
                      </div>
                    </div>
                  ))}
            ...
            <Card className="dark:bg-kvotizza-dark-bg-20">
              ...
              <CardContent className="p-2 sm:p-6 dark:bg-kvotizza-dark-bg-20">
                {/* Mobile View */}
                        ...
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <div className="text-right">
                            {summary.allAvailable ? (
                              <Badge className="text-xs bg-kvotizza-green-600 hover:bg-kvotizza-green-700">
                                Kompletno
                              </Badge>
                            ) : (
                            ...
                            <div className="flex-1 bg-muted rounded-full h-2">
                              <div
                                className={`h-2 rounded-full ${
                                  summary.allAvailable
                                    ? 'bg-kvotizza-green-500'
                                    : 'bg-kvotizza-blue-500'
                                }`}
                                style={{
                                  width: `${getCompletionRate(
                                    summary.availableSelections,
                                    selections.length
                                  )}%`,
                                }}
                              />
                        ...
                        <div className="flex items-center justify-between mt-2 pt-2 border-t">
                          ...
                          <div className="text-right">
                            ...
                            <p className="text-xs text-muted-foreground">
                              Bonus: {formatNumberEuropean(bonusAmount)} rsd
                            </p>
                          </div>
                        </div>
                      </div>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                {bonus?.conditionDescription && (
                                  <Tooltip>
                                    ...
                                    <TooltipContent>
                                      <p className="max-w-xs">{bonus.conditionDescription}</p>
                                    </TooltipContent>
                                  </Tooltip>
                                )}
                              </div>
                        ...
                        >
                          <a
                            href={outUrl(bookieOdd.bookie, {
                              ...
                              odds: bookieOdd.available
                                ? Number(bookieOdd.odds?.toFixed(2))
                                : undefined,
                            })}
                            className="block"
                          ...
                          >
                            ...
                            <p className="font-bold">
                              {bookieOdd.available ? bookieOdd.odds?.toFixed(2) : 'N/A'}
                            </p>
                          </a>
                        </div>
                      ))}
